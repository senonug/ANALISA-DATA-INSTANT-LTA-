...
def cek_indikator(row):
    ...
    indikator['urutan_fasa_terbalik'] = row.get('PHASE_SEQUENCE', 'N') == 'K-L'
...
with tab1:
    ...
    if os.path.exists(data_path):
        ...
        grouped = result.groupby('LOCATION_CODE').agg(lambda x: sum(x) if x.dtype == 'int64' else x.iloc[0])
        grouped['Jumlah Potensi TO'] = grouped.sum(axis=1)
        top50 = grouped.sort_values(by='Jumlah Potensi TO', ascending=False).head(50).reset_index()

        col1, col2, col3 = st.columns(3)
        col1.metric("📄 Total Data", len(df))
        col2.metric("🔢 Total IDPEL Unik", df['LOCATION_CODE'].nunique())
        col3.metric("🎯 Potensi Target Operasi", sum(grouped['Jumlah Potensi TO'] > 0))
